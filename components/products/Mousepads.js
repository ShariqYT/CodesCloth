            ...
            <div className="container px-5 py-24 mx-auto">
                <div className="flex flex-wrap -m-4 justify-center">
                    {Object.keys(products).length === 0 && <div className='flex flex-col justify-center items-center'>
                        <svg className='w-20' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" color="#000000" fill="none">
                        ...
                        <p className='min-h-screen'>Sorry all the Mousepads are currently out of stock. New stock coming soon. Stay Tuned!</p>
                    </div>}
                    {Object.keys(products).map((item) => {
                        return (
                            <Link key={item} href={`/product/${products[item].slug}`} className="lg:w-1/5 md:w-1/2 shadow-lg m-6 border border-gray-200 p-4 w-full">
                                <div className="flex justify-center text-black items-center relative rounded overflow-hidden">
                                    <Image unoptimized quality={100} width={1} height={1} className="w-52 text-black" priority={true} src={products[item].img} alt="tshirt" />
                                </div>
                                <div className="mt-4">
                                    <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">Mousepads</h3>
                                    ...
                                    <div className='mt-1'>
                                        ...
                                        {products[item].color.includes('pink') && <button className="border-2 border-gray-300 ml-1 bg-pink-300 rounded-full w-6 h-6 focus:outline-none"></button>}
                                    </div>
                                </div>
                            </Link>
                        )
