...
import { deletePromocodes } from '@/actions/Admin/deletePromocodes';
...
const PromoCodes = () => {
  ...
  useEffect(() => {
    document.title = "Admin - Promo Codes | CodesCloth";
  ...
  const handleDeletePromocode = async (promoCodeId) => {
    try {
      await deletePromocodes(promoCodeId);
      setPromocodes(prevPromocodes => {
        return prevPromocodes.filter(promo => promo._id !== promoCodeId);
      });
      toast.success('Promo code deleted successfully!', { duration: 5000, style: { border: '2px solid green', padding: '15px 20px', marginBottom: '40px' } });
    } catch (error) {
      throw new Error(error);
    }
  };
        ...
        <div className='overflow-x-auto'>
          <table className='w-full'>
            <thead className='text-center'>
              <tr className=''>
                <th scope="col" className="p-5 text-sm leading-6 font-semibold  capitalize rounded-t-xl"> # </th>
                <th scope="col" className="p-5 text-sm leading-6 font-semibold  capitalize"> Codes </th>
                <th scope="col" className="p-5 text-sm leading-6 font-semibold  capitalize"> Discount Amount </th>
                <th scope="col" className="p-5 text-sm leading-6 font-semibold  capitalize"> Discount Type </th>
                <th scope="col" className="p-5 text-sm leading-6 font-semibold  capitalize"> Used </th>
                <th scope="col" className="p-5 text-sm leading-6 font-semibold  capitalize"> Limit </th>
                <th scope="col" className="p-5 text-sm leading-6 font-semibold  capitalize"> Active </th>
                <th scope="col" className="p-5 text-sm leading-6 font-semibold  capitalize"> All Products </th>
                <th scope="col" className="p-5 text-sm leading-6 font-semibold  capitalize"> Expiry </th>
                <th scope="col" className="p-5 text-sm leading-6 font-semibold  capitalize rounded-t-xl"> Actions </th>
              </tr>
            </thead>
            <tbody className="divide-y text-center divide-purple-700 ">
              {promocodes?.map((promoCode, index) => (
                <tr key={index} className=" transition-all duration-200 ">
                  <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium  "> {index + 1} </td>
                  <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium "> {promoCode.code} </td>
                  <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium ">
                    {promoCode.discountType === 'percentage' ? `${promoCode.discountAmount}%` : `₹ ${promoCode.discountAmount}`} </td>
                  <td className="p-5 whitespace-nowrap text-sm leading-6 capitalize font-medium "> {promoCode.discountType} </td>
                  <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium "> {promoCode.used} </td>
                  <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium "> {promoCode.limit} </td>
                  <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium "> <input readOnly type="checkbox" checked={promoCode.active} /> </td>
                  <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium "> <input readOnly type="checkbox" checked={promoCode.allProducts} /> </td>
                  <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium">
                    {promoCode.expiry instanceof Date
                      ? promoCode.expiry.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
                      : new Date(promoCode.expiry).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
                    }
                  </td>
                  <td className=" p-5 ">
                    <div className="flex items-center justify-center gap-1">

                      <button onClick={() => handleDeletePromocode(promoCode._id)} className="p-2 rounded-full hover:scale-[1.2]  group transition-all duration-200 ease-in-out flex item-center">
